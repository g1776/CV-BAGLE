...
def pie_chart(pred, truth):
    ...
    truth_pcts = sorted(truth.data / truth.data.sum(), reverse=True)

    # get areas of predicted glyphs
    pred_glyph_areas = [cv2.contourArea(g["contour"]) for g in pred.glyphs.large]

    # keep the top n glyphs by area (where n is the number of pie slices)
    top_n_pred_glyph_areas = sorted(pred_glyph_areas, reverse=True)[:len(truth_pcts)]

    pred_pcts = np.array(top_n_pred_glyph_areas) / sum(top_n_pred_glyph_areas)

    # print("true: ", truth_pcts)
    # print("pred: ", pred_pcts)

    labels_metric = len(list(set(truth.labels) & set(pred.labels))) / len(set(truth.labels))


    pts = pred.glyphs.large
